...
function total_a_pagar() {
    ...
    //Para determinar que el usuario no escriba números o simbolos en la celda del nombre//
       const nombreValido = nombre => {
      return  /^[a-zA-ZÀ-ÿ\s]{1,40}$/.test(nombre);
      }
    if (!nombreValido(nombre.value)){
        alert("Por favor ingrese sólo texto");
        nombre.classList.add("is-invalid");
        nombre.focus();
        return;
    }

     //Para determinar que el usuario no escriba números o simbolos en la celda del apellido//
    if (apellido.value === "" ) {
